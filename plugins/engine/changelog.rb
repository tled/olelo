...
Application.hook :layout_xml do |name, xml|
  xml.sub!('</head>', %{<link rel="alternate" href="#{escape_html absolute_path('/', :output => 'changelog', :format => 'atom')}"
                        ...
                        type="application/rss+xml" title="Sitewide RSS Changelog"/></head>}.unindent)
  xml.sub!('</head>', %{<link rel="alternate" href="#{escape_html(absolute_path(page, :output => 'changelog', :format => 'atom'))}"
                        ...
                        type="application/rss+xml" title="#{escape_html page.path} RSS Changelog"/></head>}.unindent) if page && !page.new? && !page.root?
end
